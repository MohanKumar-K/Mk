...
def populate():
    ...
    print("Adding Skills with Code Snippets...")
    snippets = {
    ...
    def get_snippet(name):
        return snippets.get(name, f"// {name} init")

    cat_front = SkillCategory.objects.create(name="Frontend", icon_class="fas fa-palette", order=1)
    Skill.objects.create(category=cat_front, name="HTML", icon="🌐", order=1, code_snippet=get_snippet("HTML"))
    Skill.objects.create(category=cat_front, name="CSS", icon="🎨", order=2, code_snippet=get_snippet("CSS"))
    Skill.objects.create(category=cat_front, name="Bootstrap", icon="📐", order=3, code_snippet=get_snippet("Bootstrap"))
    Skill.objects.create(category=cat_front, name="JavaScript", icon="⚡", order=4, code_snippet=get_snippet("JavaScript"))

    cat_back = SkillCategory.objects.create(name="Backend", icon_class="fas fa-server", order=2)
    Skill.objects.create(category=cat_back, name="Python", icon="🐍", order=1, code_snippet=get_snippet("Python"))
    Skill.objects.create(category=cat_back, name="Django", icon="🚀", order=2, code_snippet=get_snippet("Django"))
    Skill.objects.create(category=cat_back, name="Java", icon="☕", order=3, code_snippet=get_snippet("Java"))

    cat_db = SkillCategory.objects.create(name="Database", icon_class="fas fa-database", order=3)
    Skill.objects.create(category=cat_db, name="MySQL", icon="🗄️", order=1, code_snippet=get_snippet("MySQL"))

    cat_other = SkillCategory.objects.create(name="Other Technologies", icon_class="fas fa-tools", order=4)
    Skill.objects.create(category=cat_other, name="IoT", icon="📡", order=1, code_snippet=get_snippet("IoT"))
    Skill.objects.create(category=cat_other, name="Embedded Systems", icon="🔌", order=2, code_snippet=get_snippet("Embedded Systems"))
    Skill.objects.create(category=cat_other, name="Git", icon="🔀", order=3, code_snippet=get_snippet("Git"))
